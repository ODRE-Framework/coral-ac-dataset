...
alignments_df = create_empty_dataframe("Folder", "GlobalID","NL","RLS","ODRL","ODRE")
for folder, file_list in files.items():
    print(folder)
    nl = read_file_to_string(base_dir+"/"+folder+"/policy.txt")
    rls = read_file_to_string(base_dir+"/"+folder+"/rls.sql")
    odrl = read_file_to_string(base_dir+"/"+folder+"/odrl.jsonld")
    odre = read_file_to_string(base_dir+"/"+folder+"/odre.json")
    global_id = get_uid_from_json_string(odrl)
    add_row(alignments_df, [folder, global_id,  nl, rls, odrl, odre])
   

print(alignments_df)

export_dataframe(alignments_df,csv_path="policy-alignments.csv", xls_path="policy-alignments.xlsx")